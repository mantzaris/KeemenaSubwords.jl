...
## 20) End-to-end user workflow tests, concurrency-safe downloads, and one-call UX

### 20.1 Objective
Now that CI is green and you have both offline-stable tests and a download-smoke lane, the next step is to make the project feel "production stable" by:

1) Making the artifact-vs-fallback decision transparent to users (informative, not scary), without stopping or changing the normal flow.
2) Hardening the fallback downloader against concurrent prefetch/install attempts (multi-thread and multi-process).
3) Adding a small set of clearly separated end-to-end (E2E) workflow tests that simulate what real users do from start to finish across multiple tokenizer families, including HF tokenizer.json.
4) Reducing the number of function calls required for common workflows so typical usage can be one call when desired (while keeping the existing API intact).
5) Recording deferred "future work" items (including gated install checksum persistence and training) in a root-level `futurework.md`.

### 20.2 Current state and what to improve
- Your tests already demonstrate that HF tokenizer.json pipelines can be loaded and used for tokenization tasks (encode/decode/tokenize, added tokens, template processing, bytelevel fixtures, etc).
- The download-smoke job proves that on a clean machine the package can download and load representative real models, which is a big step.
- What is still missing is a set of large, obvious "user story" tests that stand out as end-to-end workflows (not just unit fixtures). These help contributors and users trust that typical usage patterns remain stable.

### 20.3 Tasks

#### A) Make artifact-vs-fallback decisions informative (without scary output)
Goal: If artifact install fails but fallback succeeds, users should see a short, helpful info message rather than raw artifact failure spam, and they should be able to query what happened.

Tasks:
1) Add an "asset decision summary" message (info-level)
- When `prefetch_models_status` returns `method=:fallback_download` for a key:
  - emit a single `@info` line like:
    - "Model :mistral_v3_sentencepiece: artifact unavailable, used fallback download, cached at <path>"
  - do this once per session per model key (avoid repeated prints).
- When `method=:artifact`, stay quiet by default (or optionally print only in debug mode).

2) Capture and suppress raw artifact failure noise when fallback succeeds
- Wrap the artifact install attempt in an output-capture scope:
  - capture stdout/stderr from `Pkg.Artifacts.ensure_artifact_installed`
  - if the artifact succeeds: do nothing
  - if it fails but fallback succeeds: discard captured output and show the short info message
  - if both fail: show captured output in the thrown/warned error (plus your own summary)

3) Make the decision queryable
- Ensure `prefetch_models_status` is documented as the official "what happened?" API.
- Add a convenience:
  - `asset_status(key)::NamedTuple` (calls `prefetch_models_status([key])[key]`)
  - optional: `print_asset_status(key)` for nice formatting.

Acceptance:
- No scary artifact failure spam in normal successful fallback scenarios.
- Users can always determine method/path via `prefetch_models_status` or `asset_status`.

#### B) Concurrency hardening for fallback downloads and cache installs
Goal: If two processes (or two tasks) prefetch the same model concurrently, they should not corrupt the cache or race on temp paths.

Tasks:
1) Add per-model lock around fallback downloads
- Use a lockfile strategy in the cache root:
  - `locks/<key>.lock`
- Acquire lock using atomic file creation (open with "x" or mkdir lock dir).
- Waiting behavior:
  - poll with short sleep/backoff
  - hard timeout (configurable, default e.g. 2 minutes)
- Always release in `finally`.

2) Use unique temp filenames for downloads
- Replace fixed `dest * ".download"` with unique temp paths, e.g.:
  - `dest * ".download.<pid>.<random>"`
- After successful download + checksum:
  - atomic rename into the final destination
- Clean-up:
  - remove stale temp files on failure
  - optionally clear old temp files on next successful run

3) Make file writes atomic
- For each downloaded file:
  - download to temp
  - checksum temp
  - rename to final path (atomic where possible)
- This prevents partial files from appearing as "present".

4) Add deterministic offline tests for the lock + temp scheme
- Add tests that:
  - start two tasks trying to "install" the same fake model directory
  - use a mocked download that just writes delayed content
  - assert that final files are correct and not partially written
(If mocking is too invasive, test the lock helper directly by having tasks coordinate around it.)

Acceptance:
- No cache corruption under concurrent prefetch.
- Temp files do not collide.
- Tests validate the lock + atomic write behavior without requiring network.

#### C) Add clearly separated end-to-end (E2E) "user workflow" tests
Goal: Have obvious "start to finish" testsets that simulate typical user actions, separate from unit tests.

Tasks:
1) Add `test/e2e_user_workflows.jl` and include it from runtests
- Create a top-level testset like:
  - "E2E user workflows"
- Keep these tests high-level and readable.

2) Offline E2E workflows (always run)
Use fixtures and shipped tiny models so the tests are deterministic:
- Workflow 1: HF tokenizer.json directory
  - load by directory autodetect
  - encode, decode, tokenize
  - encode_result with masks/offsets toggles (where supported)
- Workflow 2: Register local model, then load by key
  - register_local_model!(; key=:my_local_hf, path=fixture_dir, format=:hf_tokenizer_json)
  - load_tokenizer(:my_local_hf)
  - encode and decode
- Workflow 3: WordPiece from vocab.txt fixture
  - load_tokenizer(path; format=:wordpiece_vocab)
  - encode/decode smoke
- Workflow 4: SentencePiece discovery by filename and by "single *.model" heuristic
  - tempdir with `spiece.model` and `custom.model` cases
  - load_tokenizer(dir) and ensure it selects sentencepiece_model path

3) Network E2E workflows (download-smoke gated)
When `KEEMENA_TEST_DOWNLOADS=1`:
- Workflow 5: Built-in key one-shot
  - prefetch -> load -> encode_result -> decode
  - run for the representative list already used in smoke tests:
    - tiktoken
    - gpt2 bpe
    - bert wordpiece
    - t5 sentencepiece unigram
    - qwen hf tokenizer.json

Acceptance:
- These tests read like real user stories and can be referenced in docs and PR reviews.

#### D) Reduce the number of user calls: add one-call convenience APIs
Goal: A user should be able to tokenize in one call (optional), while keeping existing explicit flows.

Tasks:
1) Add one-call overloads for the common operations
Add methods (non-breaking) that accept either a model key or a path:
- `encode(key::Symbol, text; kwargs...) -> Vector{Int}`
- `encode(path::AbstractString, text; format=nothing, kwargs...) -> Vector{Int}`
- `encode_result(key_or_path, text; kwargs...) -> TokenizationResult`
- `tokenize(key_or_path, text; kwargs...) -> Vector{String}`
- `decode(key_or_path, ids; kwargs...) -> String`

Implementation guidance:
- Internally call:
  - `tok = get_tokenizer_cached(key_or_path; format=..., prefetch=true)`
  - then call the existing `encode(tok, text)` / `encode_result(tok, text)` / etc.

2) Add a simple tokenizer cache for session reuse
- Add an internal cache:
  - `const _TOKENIZER_CACHE = Dict{Any,Any}()`
  - protect with a `ReentrantLock`
- Cache key should include:
  - model key OR absolute path
  - format override (if provided)
  - add_special_tokens default (if it affects tokenizer instance)
- Provide:
  - `clear_tokenizer_cache!()`
  - `cached_tokenizers()` (optional)

3) Add E2E tests for the one-call APIs
- Ensure a user can do:
  - `encode(:core_bpe_en, "hello")` without calling `load_tokenizer` explicitly
  - `encode(fixture_dir, "hello")` with autodetect

Acceptance:
- Most common use cases are 1 call.
- Power users can still prefetch/load explicitly.

#### E) Add `futurework.md` to record deferred goals
Goal: track important future goals without mixing them into the active plan.

Tasks:
1) Create `futurework.md` at repo root with sections:
- Distribution and reproducibility
  - compute and persist checksums for gated installs (store computed sha256 after install in local registry)
  - optionally publish artifact tarballs as GitHub Releases
- Tokenizer training (deferred)
- HF tokenizer.json parity expansion (more components, better offsets)
- Performance roadmap (caches, tries, DP optimizations)
- Expanded conformance goldens vs reference implementations

Acceptance:
- Future work is documented in one place and kept out of current tasks.

### 20.4 Suggested implementation order
1) Concurrency hardening (lock + unique temp + atomic writes) + offline tests.
2) Artifact output capture + informative messages (no scary spam).
3) Add one-call convenience APIs + minimal caching + tests.
4) Add E2E user workflow tests (offline + gated network).
5) Add futurework.md.


## 21) Strengthen real-world test coverage: shared text corpus, cross-tokenizer E2E scenarios, and stronger HF JSON assertions

### 21.1 Objective
Increase confidence that KeemenaSubwords behaves correctly in typical user situations across:
- BPE / Byte-level BPE
- WordPiece
- SentencePiece (Unigram + BPE compatibility)
- tiktoken
- Hugging Face tokenizer.json pipelines

Specifically:
1) Expand the diversity of tested input text (Unicode, whitespace, emoji, mixed scripts, long inputs).
2) Add a small number of clearly separated "full workflow" tests that stand out from unit tests.
3) Strengthen assertions for representative models beyond "non-empty ids" so drift is caught early.

### 21.2 Tasks

#### A) Add a shared "tokenization edge case corpus" used across testsets
Goal: A single corpus file drives many tests so you automatically get broad text coverage across formats.

1) Create `test/corpus/tokenization_edge_cases.toml` (or JSON) containing 100 to 250 strings.
Suggested categories (include at least a few each):
- ASCII baseline: "hello", "hello world", punctuation-heavy
- whitespace: leading spaces, multiple spaces, tabs, newlines, trailing spaces
- unicode: accented chars, combining marks, NFKC-sensitive examples
- emoji and symbols: common emoji, skin tone modifiers, variation selectors
- mixed scripts: Latin + Greek + Cyrillic + CJK in the same string
- RTL edge: a few Arabic/Hebrew strings (even if not perfect offsets, should not crash)
- long input: 8k to 32k character synthetic text
- special token strings: strings containing tokens that could match added_tokens, e.g. "[CLS]" "<s>" "
